...
game_sec_db = base.d.Database(base.GAME_SEC_DB_PATH, factory=base.ForeignKeyConnection)
game_ter_db = base.d.Database(base.GAME_TER_DB_PATH, factory=base.ForeignKeyConnection)
db_dist = itertools.cycle([game_db, game_sec_db, game_ter_db])
...
async def game_list():
	...
	games = await next(db_dist).fetch_all(query=query_str)
...
async def game_state():
	...
	try:
		...
		answer = await next(db_dist).fetch_one(query=query_str)
		...
		guess_list = await next(db_dist).fetch_all(query=query_str)
